    ...
    <Layout>
        <div className="row container max-w-7xl mt-5 product-detail-page">
            <div className="col-md-6 mx-auto mb-3" style={{width: '20rem'}}>
                <img src={`${process.env.REACT_APP_API}/api/v1/product/product-photo/${product._id}`} className="card-img-top max-h-[500px] object-contain" alt={product.name} />
            </div>
        ...
        <div className="row container m-3 product-detail-page">
            ...
            {relatedProducts?.map((p) =>(         
              <div className="card m-3" key={p._id} style={{width: '18rem'}}>
                  <img src={`${process.env.REACT_APP_API}/api/v1/product/product-photo/${p._id}`} className="card-img-top card-img-fixed" alt={p.name} />
